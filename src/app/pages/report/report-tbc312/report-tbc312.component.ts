...
@Component({
  selector: 'app-report-tbc312',
  templateUrl: './report-tbc312.component.html',
  styleUrls: ['./report-tbc312.component.scss']
})
export class ReportTbc312Component {
  ...
  reportName: any = 'สมุดทะเบียนคืนเบี้ยประกันภัย';
  reportShortName: any = 'ทบ.ช.3.12';
  workbook: any = null;
  toast = Swal.mixin({
    toast: true,
    position: 'bottom-end',
  ...
  ngOnInit(): void {
    ...
    this.currentBranch$.subscribe((res) => {
      ...
      if (res && res.length && abbR_NAME && !this.isGetMaxDate) {
        this.dashboardService.getReportDataOIC011MaxDate(!abbR_NAME ? '0' : abbR_NAME).subscribe((res: any) => {
  ...
  downloadClick(event: any) {
    ...
    this.dashboardService.getReportDataOIC011s(`${this.fromDate?.year! - 543}-${padStart(this.fromDate?.month + '', 2, '0')}-${padStart(this.fromDate?.day + '', 2, '0')}`, `${this.toDate?.year! - 543}-${padStart(this.toDate?.month + '', 2, '0')}-${padStart(this.toDate?.day + '', 2, '0')}`, this.branch).subscribe((res: any) => {
      if (res && res.length) {
        this.genExcel(res);
      } else {
  ...
  async genExcel(data: any) {
    ...
    let row = 1;
    let cell;
    let columnMap = [{ excelColumn: 'A', dataColumn: 'applicatioN_DATE', isDate: true, width: 17, formula: false }
      , { excelColumn: 'B', dataColumn: 'canceL_REASON', width: 50 }
      , { excelColumn: 'C', dataColumn: 'policY_NO', width: 17 }
      , { excelColumn: 'D', dataColumn: 'commencemenT_DATE', isDate: true, width: 17 }
      , { excelColumn: 'E', dataColumn: 'coveR_END_DATE', isDate: true, width: 17 }
      , { excelColumn: 'F', dataColumn: 'maiN_PLAN_NAME', width: 50 }
      , { excelColumn: 'G', dataColumn: 'doB_OF_INSURED', isDate: true, width: 17 }
      , { excelColumn: 'H', dataColumn: 'iD_NO_OF_INSURED', width: 17 }
      , { excelColumn: 'I', dataColumn: 'insureD_NAME', width: 30 }

      , { excelColumn: 'J', dataColumn: 'sA_MAIN_BENEFIT', isNumber: true, width: 15 }
      , { excelColumn: 'K', dataColumn: 'sA_ACC_RIDER', isNumber: true, width: 15 }
      , { excelColumn: 'L', dataColumn: 'sA_HEALTH_RIDER', isNumber: true, width: 15 }
      , { excelColumn: 'M', dataColumn: 'sA_OTHER_RIDER', isNumber: true, width: 15 }
      , { excelColumn: 'N', dataColumn: 'sA_TOTAL', isNumber: true, width: 15 }
      , { excelColumn: 'O', dataColumn: 'premiuM_MAIN_BENEFIT', isNumber: true, width: 15 }
      , { excelColumn: 'P', dataColumn: 'premiuM_ACC_RIDER', isNumber: true, width: 15 }
      , { excelColumn: 'Q', dataColumn: 'premiuM_HEALTH_RIDER', isNumber: true, width: 15 }
      , { excelColumn: 'R', dataColumn: 'premiuM_OTHER_RIDER', isNumber: true, width: 15 }
      , { excelColumn: 'S', dataColumn: 'premiuM_TOTAL', isNumber: true, width: 15 }
      , { excelColumn: 'T', dataColumn: 'totaL_REFUND_AMOUNT', isNumber: true, width: 15 }
      , { excelColumn: 'U', dataColumn: 'paymenT_DATE', isDate: true, width: 17 }
      // , { excelColumn: 'V', dataColumn: 'chequE_NO', width: 60 }
      // , { excelColumn: 'W', dataColumn: '', width: 17 }
      // , { excelColumn: 'X', dataColumn: '', isDate: true, width: 17 }
      // , { excelColumn: 'Y', dataColumn: '', width: 17 }

      , { excelColumn: 'V', dataColumn: 'inserT_BY', width: 15 }
      , { excelColumn: 'W', dataColumn: 'createD_BY_USERNAME', width: 18 }
      , { excelColumn: 'X', dataColumn: 'createD_DATE', width: 17, isDate: true }
      , { excelColumn: 'Y', dataColumn: 'abbR_NAME', width: 15 }
      , { excelColumn: 'Z', dataColumn: 'companY_NAME', width: 30 }
      , { excelColumn: 'AA', dataColumn: 'applicatioN_BRANCH_ABBR_NAME', width: 15 }
      , { excelColumn: 'AB', dataColumn: 'applicatioN_BRANCH_NAME', width: 30 }
    ];
    // data = orderBy(data, ['applicatioN_BRANCH_NAME', 'applicatioN_DATE', 'policY_CODE'], ['asc', 'asc', 'asc']);
    data = orderBy(data, ['applicatioN_DATE'], ['asc']);
  ...
  createSheet(sheetName: any, columnMap: any, row: any, element: any, elementCode: any, state: any, it: any, dataByBranch: any, isOIC: any = true, isAllData = true) {
    ...
    worksheet.mergeCells(`A${row}:AD${row + 1}`);
    ...
    worksheet.mergeCells(`A${row}:AD${row}`);
    cell = worksheet.getCell(`A${row}`);
    ...
    cell = worksheet.getCell(`A${row}`);
    cell.alignment = { vertical: 'middle', horizontal: 'left' };
    cell.value = `วันที่บอกล้าง ${padStart(this.fromDate?.day + '', 2, '0')}/${padStart(this.fromDate?.month + '', 2, '0')}/${this.fromDate?.year} ถึง ${padStart(this.toDate?.day + '', 2, '0')}/${padStart(this.toDate?.month + '', 2, '0')}/${this.toDate?.year}`;
    row++;
    ...
    this.headerMergeRow(worksheet, 'B', row, 'เหตุที่บอกล้าง', 2);
    this.headerMergeColumn(worksheet, 'C', row, 'กรมธรรม์ประกันภัย', 'E');
    this.headerMergeRow(worksheet, 'C', row + 1, 'เลขที่กรมธรรม์', 1);
    this.headerMergeRow(worksheet, 'D', row + 1, 'วันที่เริ่มคุ้มครอง', 1);
    this.headerMergeRow(worksheet, 'E', row + 1, 'วันสิ้นสุดการคุ้มครอง', 1);
    this.headerMergeRow(worksheet, 'F', row, 'แบบการประกันภัย', 2);
    this.headerMergeRow(worksheet, 'G', row, 'วันเดือนปีเกิด', 2);
    this.headerMergeRow(worksheet, 'H', row, 'เลขที่บัตรประชาชน', 2);
    this.headerMergeRow(worksheet, 'I', row, 'ชื่อ นามสกุลผู้เอาประกัน', 2);
    this.headerMergeColumn(worksheet, 'J', row, 'จำนวนเงินเอาประกัน', 'N');
    this.headerMergeRow(worksheet, 'J', row + 1, 'กรมธรรม์หลัก', 1);
    this.headerMergeRow(worksheet, 'K', row + 1, 'อุบัติเหตุ', 1);
    this.headerMergeRow(worksheet, 'L', row + 1, 'สุขภาพ', 1);
    this.headerMergeRow(worksheet, 'M', row + 1, 'อื่น', 1);
    this.headerMergeRow(worksheet, 'N', row + 1, 'รวม', 1);
    this.headerMergeColumn(worksheet, 'O', row, 'จำนวนเงินเบี้ยประกัน', 'S');
    this.headerMergeRow(worksheet, 'O', row + 1, 'กรมธรรม์หลัก', 1);
    this.headerMergeRow(worksheet, 'P', row + 1, 'อุบัติเหตุ', 1);
    this.headerMergeRow(worksheet, 'Q', row + 1, 'สุขภาพ', 1);
    this.headerMergeRow(worksheet, 'R', row + 1, 'อื่น', 1);
    this.headerMergeRow(worksheet, 'S', row + 1, 'รวม', 1);

    this.headerMergeRow(worksheet, 'T', row, 'จำนวนเงินที่จ่ายคืน', 2);
    this.headerMergeRow(worksheet, 'U', row, 'วัน เดือน ปี ที่จ่าย', 2);

    // this.headerMergeRow(worksheet, 'V', row, 'เลขที่เอกสาร', 2);
    // this.headerMergeRow(worksheet, 'W', row, 'ผู้รับ', 2);
    // this.headerMergeRow(worksheet, 'X', row, 'วันที่รับเงิน', 2);
    // this.headerMergeRow(worksheet, 'Y', row, 'หมายเหตุ ', 2);
    if (isOIC) {

      this.headerMergeRow(worksheet, 'V', row, 'User id', 2);
      this.headerMergeRow(worksheet, 'W', row, 'User name', 2);
      this.headerMergeRow(worksheet, 'X', row, 'Create date', 2);
      this.headerMergeRow(worksheet, 'Y', row, 'User branch code', 2);
      this.headerMergeRow(worksheet, 'Z', row, 'User branch name', 2);
      this.headerMergeRow(worksheet, 'AA', row, 'Received Branch code', 2);
      this.headerMergeRow(worksheet, 'AB', row, 'Received Branch name', 2);
    }
    row++;
    row++;
    row++;
    ...
    let columnSum = 'F';
    this.headerMergeColumn(worksheet, 'C', row, 'จำนวนกรมธรรม์', 'E');
    ...
    columnSum = 'N';
    this.headerMergeColumn(worksheet, 'J', row, 'จำนวนเงินรวมเงินเอาประกัน', 'M');
    cell = worksheet.getCell(`${columnSum}${row}`);
    ...
    columnSum = 'S';
    this.headerMergeColumn(worksheet, 'O', row, 'จำนวนเงินรวมเงินเบี้ยประกัน ', 'R');
    cell = worksheet.getCell(`${columnSum}${row}`);
    cell.value = { formula: `=SUM(${columnSum}${rowStart}:${columnSum}${row - 1})`, date1904: false };
    this.setBorderAll(cell);
    this.setNumberFormat(cell);
    ...
    columnSum = 'T';
    //this.headerMergeColumn(worksheet, 'O', row, 'จำนวนงินรวมที่จ่ายคืนสุทธิ ', 'R');
    cell = worksheet.getCell(`${columnSum}${row}`);
    cell.value = { formula: `=SUM(${columnSum}${rowStart}:${columnSum}${row - 1})`, date1904: false };
    this.setBorderAll(cell);
    this.setNumberFormat(cell);
