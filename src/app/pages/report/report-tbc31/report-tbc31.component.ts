...
@Component({
  selector: 'app-report-tbc31',
  templateUrl: './report-tbc31.component.html',
  styleUrls: ['./report-tbc31.component.scss']
})
export class ReportTbc31Component {
  ...
  reportName: any = 'สมุดทะเบียนการจ่ายเงินตามกรมธรรม์ประกันภัย-มรณกรรม';
  reportShortName: any = 'ทบ.ช.3.1';
  workbook: any = null;
  toast = Swal.mixin({
    toast: true,
    position: 'bottom-end',
  ...
  ngOnInit(): void {
    ...
    this.currentBranch$.subscribe((res) => {
      ...
      if (res && res.length && abbR_NAME && !this.isGetMaxDate) {
        this.dashboardService.getReportDataOIC003MaxDate(!abbR_NAME ? '0' : abbR_NAME).subscribe((res: any) => {
  ...
  downloadClick(event: any) {
    ...
    const dateSent = moment(this.date).add(-543, 'y');
    this.dashboardService.getReportDataOIC003s(`${this.fromDate?.year! - 543}-${padStart(this.fromDate?.month + '', 2, '0')}-${padStart(this.fromDate?.day + '', 2, '0')}`, `${this.toDate?.year! - 543}-${padStart(this.toDate?.month + '', 2, '0')}-${padStart(this.toDate?.day + '', 2, '0')}`, this.branch).subscribe((res: any) => {
      if (res && res.length) {
        this.genExcel(res);
      } else {
  ...
  async genExcel(data: any) {
    ...
    let columnMap = [{ excelColumn: 'A', dataColumn: 'casE_NO', width: 17 }
      ...
      , { excelColumn: 'C', dataColumn: 'deatH_DATE', isDate: true, width: 17 }
      , { excelColumn: 'D', dataColumn: 'deatH_REASON', width: 40 }
      , { excelColumn: 'E', dataColumn: 'policY_CODE', width: 17 }
      , { excelColumn: 'F', dataColumn: 'validatE_DATE', isDate: true, width: 17 }
      , { excelColumn: 'G', dataColumn: 'expirE_DATE', isDate: true, width: 17 }
      , { excelColumn: 'H', dataColumn: 'producT_NAME', width: 45 }
      , { excelColumn: 'I', dataColumn: 'insureD_BIRTHDAY', isDate: true, width: 17 }
      , { excelColumn: 'J', dataColumn: 'insureD_ID_NO', width: 30 }
      , { excelColumn: 'K', dataColumn: 'insureD_NAME', width: 30 }
      , { excelColumn: 'L', dataColumn: 'beneficiarY_NAME', width: 60 }
      , { excelColumn: 'M', dataColumn: 'maiN_SA', isNumber: true, width: 15 }
      , { excelColumn: 'N', dataColumn: 'acC_RIDER_SA', isNumber: true, width: 15 }
      , { excelColumn: 'O', dataColumn: 'healtH_RIDER_SA', isNumber: true, width: 15 }
      , { excelColumn: 'P', dataColumn: 'otheR_RIDER_SA', isNumber: true, width: 15 }
      , { excelColumn: 'Q', dataColumn: '', formula: '=SUM(M0:P0)', isNumber: true, width: 15 }
      , { excelColumn: 'R', dataColumn: 'maiN_PAY_AMT', isNumber: true, width: 15 }
      , { excelColumn: 'S', dataColumn: 'acC_RIDER_PAY_AMT', isNumber: true, width: 15 }
      , { excelColumn: 'T', dataColumn: 'healtH_RIDER_PAY_AMT', isNumber: true, width: 15 }
      , { excelColumn: 'U', dataColumn: 'otheR_RIDER_PAY_AMT', isNumber: true, width: 15 }
      , { excelColumn: 'V', dataColumn: '', formula: '=SUM(R0:U0)', isNumber: true, width: 15 }
      , { excelColumn: 'W', dataColumn: 'paymenT_DATE', isDate: true, width: 17 }
      // , { excelColumn: 'V', dataColumn: 'chequE_NO', width: 60 }
      // , { excelColumn: 'W', dataColumn: '', width: 15 }
      // , { excelColumn: 'X', dataColumn: '', width: 15 }
      // , { excelColumn: 'Y', dataColumn: '', width: 15 }

      , { excelColumn: 'X', dataColumn: 'createD_BY', width: 15 }
      , { excelColumn: 'Y', dataColumn: 'createD_BY_USERNAME', width: 18 }
      , { excelColumn: 'Z', dataColumn: 'createD_DATE', width: 17, isDate: true }
      , { excelColumn: 'AA', dataColumn: 'abbR_NAME', width: 15 }
      , { excelColumn: 'AB', dataColumn: 'companY_NAME', width: 30 }
      , { excelColumn: 'AC', dataColumn: 'brancH_RECEIVE_ABBR_NAME', width: 15 }
      , { excelColumn: 'AD', dataColumn: 'brancH_RECEIVE_NAME', width: 30 }
    ];
  ...
  createSheet(sheetName: any, columnMap: any, row: any, element: any, elementCode: any, state: any, it: any, dataByBranch: any, isOIC: any = true, isAllData = true) {
    ...
    this.headerMergeRow(worksheet, 'D', row, 'สาเหตุที่มรณะ', 2);
    ...
    this.headerMergeRow(worksheet, 'H', row, 'แบบประกัน', 2);
    this.headerMergeRow(worksheet, 'I', row, 'วันเดือนปีเกิด', 2);
    this.headerMergeRow(worksheet, 'J', row, 'เลขที่บัตรประชาชน', 2);
    this.headerMergeRow(worksheet, 'K', row, 'ชื่อ นามสกุลผู้เอาประกัน', 2);
    this.headerMergeRow(worksheet, 'L', row, 'ชื่อ นามสกุลผู้รับผลประโยชน์', 2);
    this.headerMergeColumn(worksheet, 'M', row, 'จำนวนเงินเอาประกัน', 'Q');
    this.headerMergeRow(worksheet, 'M', row + 1, 'กรมธรรม์หลัก', 1);
    this.headerMergeRow(worksheet, 'N', row + 1, 'อุบัติเหตุ', 1);
    this.headerMergeRow(worksheet, 'O', row + 1, 'สุขภาพ', 1);
    this.headerMergeRow(worksheet, 'P', row + 1, 'อื่น ๆ', 1);
    this.headerMergeRow(worksheet, 'Q', row + 1, 'รวม', 1);
    this.headerMergeColumn(worksheet, 'R', row, 'จำนวนเงินที่จ่าย', 'V');
    this.headerMergeRow(worksheet, 'R', row + 1, 'กรมธรรม์หลัก', 1);
    this.headerMergeRow(worksheet, 'S', row + 1, 'อุบัติเหตุ', 1);
    this.headerMergeRow(worksheet, 'T', row + 1, 'สุขภาพ', 1);
    this.headerMergeRow(worksheet, 'U', row + 1, 'อื่น ๆ', 1);
    this.headerMergeRow(worksheet, 'V', row + 1, 'รวม', 1);
    this.headerMergeRow(worksheet, 'W', row, 'วันที่จ่าย ', 2);
    // this.headerMergeRow(worksheet, 'V', row, 'เลขที่เอกสารจ่าย ', 2);
    ...
    if (isOIC) {
      this.headerMergeRow(worksheet, 'X', row, 'User id', 2);
      this.headerMergeRow(worksheet, 'Y', row, 'User name', 2);
      this.headerMergeRow(worksheet, 'Z', row, 'Create date', 2);
      this.headerMergeRow(worksheet, 'AA', row, 'User branch code', 2);
      this.headerMergeRow(worksheet, 'AB', row, 'User branch name', 2);
      this.headerMergeRow(worksheet, 'AC', row, 'Received Branch code', 2);
      this.headerMergeRow(worksheet, 'AD', row, 'Received Branch name', 2);
    ...
    this.headerMergeColumn(worksheet, 'D', row, 'จำนวนกรมธรรม์', 'F');
    cell = worksheet.getCell(`G${row}`);
    cell.value = { formula: `=COUNTA(G${rowStart}:G${row - 1})`, date1904: false };
    this.setBorderAll(cell);
    this.setNumberFormat(cell);

    this.headerMergeColumn(worksheet, 'M', row, 'จำนวนเงินรวมเงินเอาประกัน', 'P');
    cell = worksheet.getCell(`Q${row}`);
    cell.value = { formula: `=SUM(Q${rowStart}:Q${row - 1})`, date1904: false };
    this.setBorderAll(cell);
    this.setNumberFormat(cell);

    this.headerMergeColumn(worksheet, 'R', row, 'จำนวนเงินรวมเงินเบี้ยประกัน ', 'U');
    cell = worksheet.getCell(`V${row}`);
    cell.value = { formula: `=SUM(V${rowStart}:V${row - 1})`, date1904: false };
    this.setBorderAll(cell);
    this.setNumberFormat(cell);
